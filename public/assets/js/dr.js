...
var so_t = $('#tbl_sales_order_details').DataTable({
    ...
    "columnDefs": [
        ...
        {  className: "price"       , targets: 7 },
        {  className: "discount"    , targets: 8 },
        {  className: "net_price"   , targets: 9 },
        {  className: "amount"      , targets: 10 },
    ],
    "footerCallback": function ( row, data, start, end, display ) {
            ...
            //     '₱<span id="total_price">'+ numberWithCommas(t1.toFixed(2)) +'</span> Total'
            // );

            //  $( api.column( 9 ).footer() ).html(
            //     // '₱'+ numberWithCommas(t2.toFixed(2)) +' Total'
            //     '₱<span id="total_net_price">'+ numberWithCommas(t2.toFixed(2)) +'</span> Total'
            // );

            $( api.column( 10 ).footer() ).html(
                '₱ <span id="total_amount">'+ numberWithCommas(t3.toFixed(2)) +'</span>'
            );
            ...
            $.each(data, function(k,v){
                ...
                var cDesc = v.cDesc;
                    ...
                    // var nAmount = (parseFloat(v.nNetPrice*v.nBalance).toFixed(2));
                     var nPrice = numberWithCommas(parseFloat(v.nPrice).toFixed(2))
               ...
               so_t.row.add([
                    ...
                    v.cWH,
                    v.cUnit,
                    // '<input type="text" style="background: transparent; max-width: 50px; border:none; " class=" nBalance'+rows_l+'" onKeyup="updateAmount('+rows_l+')" name="nQty[\''+v.cItemNo+'\']" rel="'+rows_l+'" value="' + v.nBalance + '"  />',
                    '<input type="text" style="background: transparent; max-width: 40px; border:none; " class="text-right nBalance'+rows_l+'"   onKeyup="updateAmount('+rows_l+')" name="nQty['+v.cItemNo+']" rel="'+rows_l+'" value="' + v.nBalance + '" />',
                    // v.nPrice,
                    '<input type="text" style="background: transparent; max-width: 70px; border:none; " class="text-right nBalancePrice'+rows_l+'" onKeyup="updatePrice('+rows_l+')" name="nPrice['+v.cItemNo+']" rel="'+rows_l+'" value="' + nPrice + '" />',
                    '<input type="text" style="background: transparent; max-width: 40px; border:none; " class="text-right nBalanceDiscount'+rows_l+'" onKeyup="updateDiscount('+rows_l+')" name="nDiscount['+v.cItemNo+']" rel="'+rows_l+'" value="' + v.cDiscount + '"  />',
                    v.nNetPrice,
                    v.nAmount,
...
function updateAmount(row_id){
    var rqty = $('.nBalance'+row_id).val();
        rprice = $('.nBalance'+row_id).closest('td').next('td').find('input').val();
        rdisc = $('.nBalance'+row_id).closest('td').next('td').next('td').find('input').val();
        ...
        var tdAmountRaw = $(this).text();
        var tdAmountTxt = tdAmountRaw.replace(/,/g, "");
             if($(this).hasClass('total')) {
                 $(this).text(totalAmount);
                 totalAmount = 0;
            } else {
               totalAmount += parseFloat(tdAmountTxt);
            } 
...
function updatePrice(row_id){
    ...
    var rprice = $('.nBalancePrice'+row_id).val();
        rqty = $('.nBalance'+row_id).val();
        rdisc = $('.nBalancePrice'+row_id).closest('td').next('td').find('input').val();
        qty = parseInt(rqty.replace(/,/g, ""));
        price = parseFloat(rprice.replace(/,/g, ""));
        disc = rdisc.replace(/,/g, "");
        ...
        $('.nBalancePrice'+row_id).closest('td').parent().find('td:eq(9)').text(numberWithCommas(np.toFixed(2)));
        $('.nBalancePrice'+row_id).closest('td').parent().find('td:eq(10)').text(amt);
        $('li[data-dtr-index="10"]').find('span.dtr-data').text(amt.toLocaleString(undefined, { maximumFractionDigits: 2 }));

        //price
        let totalPrice = 0;
        $('td.price').find('input').each(function(){

        let tdPriceRaw = $(this).val();
        let tdPriceTxt = tdPriceRaw.replace(/,/g, "");
             if($(this).hasClass('total')) {
                 $(this).text(totalPrice);
                 totalPrice = 0;
            } else {
               totalPrice += parseFloat(tdPriceTxt);
            } 
        ...
        //net price
        let totalNetPrice = 0;
        $('td.net_price').each(function(){

        let tdNetPriceRaw = $(this).text();
        let tdNetPriceTxt = tdNetPriceRaw.replace(/,/g, "");
             if($(this).hasClass('total')) {
                 $(this).text(totalNetPrice);
                 totalNetPrice = 0;
            } else {
               totalNetPrice += parseFloat(tdNetPriceTxt);
            } 
        ...
        let tdAmountRaw = $(this).text();
        let tdAmountTxt = tdAmountRaw.replace(/,/g, "");
             if($(this).hasClass('total')) {
                 $(this).text(totalAmount);
                 totalAmount = 0;
            } else {
        ...
        $('#total_price').text(numberWithCommas(totalPrice.toFixed(2)));
        $('#total_net_price').text(numberWithCommas(totalNetPrice.toFixed(2)));
        $('#total_amount').text(numberWithCommas(totalAmount.toFixed(2)));
}

function updateDiscount(row_id){

    var rprice = $('.nBalancePrice'+row_id).val();
        rqty = $('.nBalance'+row_id).val();
        rdisc = $('.nBalanceDiscount'+row_id).val();
        console.log({ rprice, rqty, rdisc });
        qty = parseInt(rqty.replace(/,/g, ""));
        price = parseFloat(rprice.replace(/,/g, ""));
        disc = rdisc.replace(/,/g, "");
