...
$('#modal-inv-details').on('shown.bs.modal', function () {
   ...
   if($('#cName').val() == ''){
        ...
        $.ajax({
            url: URL + '/return_good_slip/get_si_list',
            type: "POST",
...
var rgs_t = $('#tbl_rgs_details').DataTable({
    "autoWidth": !1,
    "responsive": !0,
...
$('#submit_invoice').click(function(){
    ...
    $.ajax({
        url: URL + '/return_good_slip/get_inv_t_by_ids',
        type: "POST",
        data:{
            cItemNos: pids,
            cInvNo: cInvNo
        ...
        success: function(data){
            ...
            var html = '';
            var tds = '';
            var rgs_details = $('#rgs_details_div').html();
            var dr = '';
            var i = 0;

            //var rows_l = $('#tbl_rgs_details tbody tr').length;
            ...
            $.each(data, function(k,v){
               ...
               rgs_t.row.add([
                    cInvNo + '<input class="border-5 rgs_details" type="hidden" name="rgs_details[' + rows_l + '][cRefNo]" value="'+cInvNo+'" />',
                    v.cItemNo + '<input class="border-5 rgs_details" type="hidden" name="rgs_details[' + rows_l + '][cItemNo]" value="'+v.cItemNo+'" />',
                    cDesc + '<input class="border-5 rgs_details" type="hidden" name="rgs_details[' + rows_l + '][cDesc]"  value="'+cDesc+'" />',
                    v.cWName + '<input class="border-5 rgs_details" type="hidden" name="rgs_details[' + rows_l + '][cWH]"  value="'+v.cWH+'" />',
                    v.cUnit + '<input class="border-5 rgs_details" type="hidden" name="rgs_details[' + rows_l + '][cUnit]"  value="'+v.cUnit+'" />',
                    '<input class="border-5 rgs_details nQty'+rows_l+'" type="text" name="rgs_details[' + rows_l + '][nQty]"  value="'+v.nQty+'" onChange="updateAmount('+rows_l+')" />',
                    v.nPrice,
                    v.nAmount,
                    '<input class="border-5 rgs_details" type="text" name="rgs_details[' + rows_l + '][cBatchID]" />',
                    '<input class="border-5 rgs_details" type="text" name="rgs_details[' + rows_l + '][cRemarks]" />',
                    '<input class="border-5 rgs_details" type="hidden" name="rgs_details[' + rows_l + '][nAmount]"  value="'+v.nAmount+'" /><input class="rgs_details" type="hidden" name="rgs_details[' + rows_l + '][nPrice]"  value="'+v.nPrice+'" /><a class="ml10" href="javascript:void(0)" id="delr'+rows_l+'" onClick="openDel('+rows_l+')"><i class="icon-cross2 text-danger"></i></a>'
                ]).draw();
...
function populateInvDetails(){
    ...
    $.ajax({
        url: URL + '/return_good_slip/get_si_details',
        type: "POST",
        data:{cInvNo: cInvNo},
        dataType: 'json',
        success: function(data){
...
function updateAmount(row_id){
    var rqty = $('.nQty'+row_id).val();
        ...
        $('#tbl_rgs_details tr').each(function () {
